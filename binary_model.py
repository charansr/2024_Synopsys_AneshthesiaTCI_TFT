...
spike_detector = final_df["Orchestra/PPF20_RATE"] > (tar_mean + (0.3 * tar_std))
...
# checkpoint that saves best model based on validation loss
best_model_callback = ModelCheckpoint(
    monitor='val_loss', 
    dirpath='/home/charansr/out_dev1/checkpoints/',  
...
# checkpoint that saves all models
all_model_callback = ModelCheckpoint(
    dirpath='/home/charansr/out_dev1/checkpoints/',  
    filename='all_model-{epoch:02d}-{val_loss:.2f}',  
    save_top_k=-1,
    every_n_epochs=1
)

# Custom Logger that adds the epoch to the output.txt file
class metric_record(pl.callbacks.Callback):
    def on_epoch_start(self, trainer, pl_module):
        print(trainer.current_epoch)
        newep=str(trainer.current_epoch)
        newep+="\n"
        append_string_to_txt(newep,"/home/charansr/out_dev1/output.txt")
...
# Creating basic TensorBoardLogger logger
logger = TensorBoardLogger(save_dir="/home/charansr/out_dev1")  

# Creating Pytorch Lightning trainer object
trainer = pl.Trainer(
    max_epochs=90,
    accelerator='gpu',
    devices="auto",
    ...
    callbacks=[best_model_callback,all_model_callback,metric_record()],
    logger=logger,
